...
from autonomy_state import ScheduleTask, atomic_write, choose_launchable, load_execution_spec, parse_front_matter, read_tsv, sha256_file, upsert_tsv, write_tsv
...
TERMINAL={'PASS','VOID','STOP','INCONCLUSIVE','BLOCKED'}
...
def start(w,authorised,poll):
    ...
    wr=rows(w); st=load_status(w); [st.setdefault(r['task_id'],{**{k:'' for k in STATUS_FIELDS},'task_id':r['task_id'],'state':'PENDING'}) for r in wr]; save_status(w,st); commit([status_path(w)],'wave: start '+w.parent.name)
    workers={}; escal=False
    while True:
        ...
        for tid,(proc,rec,cost) in list(workers.items()):
            ...
            if rr.get('worker_state')=='COMPLETE': term=rr['task_state']; set_board(tid,term,'autonomous wave'); set_status(w,st,tid,term,rr.get('scientific_outcome',''),finished_at=rr.get('finished_at',utc()))
            else: term='VOID' if rr.get('worker_state') in {'REFUSED','ARTIFACT_INVALID','PROCESS_FAILED'} else 'STOP'; set_board(tid,term,rr.get('worker_state','worker failure')); set_status(w,st,tid,term,rr.get('error') or rr.get('stop_reason') or rr.get('worker_state','')); escal=True
        ...
        for tid in sel:
            ...
            set_board(tid,'RUNNING','autonomous wave'); p,rec=spawn(tid,sp,w); workers[tid]=(p,rec,int(r['io_tokens'])); ledger(tid,freeze_commit=spec['freeze_commit'],execution_commit=spec['freeze_commit'],state='RUNNING',backend=spec['backend'],primary_output=spec['output_directory'],task_report=str(Path(spec['output_directory'])/'TASK_REPORT.md'),interpretation_ceiling='SEE_TASK_LAUNCHER',started_at=utc()); set_status(w,st,tid,'RUNNING','worker launched',freeze_commit=spec['freeze_commit'],worker_pid=p.pid,started_at=utc()); commit([BOARD,LEDGER,status_path(w)],f'wave: launch {tid}')
        if not workers:
