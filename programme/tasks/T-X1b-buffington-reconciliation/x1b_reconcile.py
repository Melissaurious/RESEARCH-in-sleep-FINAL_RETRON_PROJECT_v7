#!/usr/bin/env python3
"""
T-X1b · exact-sequence reconciliation of the Buffington 2025 retron catalogue
...
SCRIPT_VERSION = "1.0.0"   # T-X1b. Minimal correction of T-X1 (VOID). See VOID_01.
SEED = 20260920
...
# ⛔ EXPLICIT. T-X1 was VOIDed by `"rt_family" if "rt_family" in cols else cols[1]`,
# which bound "family" to rt_aa_len -- the protein LENGTH -- and returned 0 Retrons.
# There is NO fallback in T-X1b: an absent expected column is a blocking STOP.
FAMILY_KEY_COL = "rt_seq_hash"
FAMILY_LABEL_COL = "family_label"
RETRON_LABEL = "Retron"
RETRON_EXPECTED = 78287
FAMILY_DISTINCT_EXPECTED = 42   # family_label currently holds 42 distinct values.
# ⛔ The historical registry name RT-FAMILY-LABELS-613 does NOT describe this
# vocabulary. The origin of 613 is unresolved and appears to belong to a different
# type_set_norm vocabulary. T-X1b does not consume, interpret or reconcile it.
...
def load_retron_ids():
    """Return (set of Retron rt_seq_hash, n_distinct family_label values).

    ⛔ NO FALLBACK. If either declared column is absent this RAISES, and the caller
    turns that into a blocking control failure. T-X1 guessed a column and produced a
    confident wrong number; guessing is the defect, not the column name."""
    import pyarrow.parquet as pq
    ft = pq.read_table(FAMILY)
    cols = ft.schema.names
    missing = [c for c in (FAMILY_KEY_COL, FAMILY_LABEL_COL) if c not in cols]
    if missing:
        raise KeyError(f"declared column(s) absent from {FAMILY}: {missing}; present: {cols}")
    labels = ft.column(FAMILY_LABEL_COL).to_pylist()
    keys = ft.column(FAMILY_KEY_COL).to_pylist()
    return ({k for k, v in zip(keys, labels) if v == RETRON_LABEL}, len(set(labels)))
...
def run_controls(out: str) -> tuple[list, bool]:
    ...
    # --- BLOCKING PRE-PRIMARY GATE: the family join, which voided T-X1 -------
    try:
        retron_ids, n_distinct = load_retron_ids()
        n_retron = len(retron_ids)
        err = ""
    except Exception as ex:
        n_retron, n_distinct, err = -1, -1, f"{type(ex).__name__}: {ex}"
    add("X1b_GATE_family_join_retron_count", "positive", "YES",
        f"{FAMILY_LABEL_COL} == '{RETRON_LABEL}' yields exactly {RETRON_EXPECTED}",
        f"{n_retron}" + (f"  ERROR {err}" if err else ""),
        "PASS" if n_retron == RETRON_EXPECTED else "FAIL",
        "⛔ THE GATE WHOSE ABSENCE VOIDED T-X1. A wrong column now stops the task "
        "BEFORE the primary, instead of surfacing as 0 afterwards")
    add("X1b_GATE_family_vocabulary_size", "positive", "YES",
        f"{FAMILY_LABEL_COL} holds exactly {FAMILY_DISTINCT_EXPECTED} distinct values",
        str(n_distinct), "PASS" if n_distinct == FAMILY_DISTINCT_EXPECTED else "FAIL",
        "recorded, not reconciled: the registry name RT-FAMILY-LABELS-613 does NOT "
        "describe this vocabulary and the origin of 613 is unresolved")
