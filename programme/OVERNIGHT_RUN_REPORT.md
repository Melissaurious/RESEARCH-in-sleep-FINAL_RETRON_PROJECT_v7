...
| **2** | **Rotate the exposed API key** | Still live in plaintext in your Claude config, in an MCP env block, and printed to a transcript. Carried over from the previous session; not inspected further, because inspecting it repeats the exposure. |
