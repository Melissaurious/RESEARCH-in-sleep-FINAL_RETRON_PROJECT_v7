...
| P4 | ⛔ **WITHDRAWN — see `tasks/T-REG-asset-registration/DISPOSITION_01`.** The 0.217% and 86.9% registry-coverage figures are **not validated measurements** and may not be cited as registry coverage. `REG_controls.tsv` records `CTL-6`, the only check on the matcher that defines them, as `blocking=NO`. They may be referred to only as the output of one unvalidated literal-path matcher, pending `T-REG2`. What survives is the inventory: **20,765 collections, 1,120,868 files, 155,622,173,070 bytes** over **five suffix-defined kinds** — a navigational census, not the size of the evidence base. |
...
| review item | bearing |
|---|---|
| **C-28, the exact-RT residual** | ⚠️ **SUPERSEDED — see `tasks/T-A0-lineage-variance/CORRECTION_01`.** The sentence "The review predicted it might not survive. It did." is **withdrawn**. The criterion did not fire, and the type-blocked interval excludes zero, but three landed cells the first draft did not cite bear directly on it: the `near_dup_sensitivity` interval **spans zero** at [−0.006009444221056286, +0.0019628428863705427] (`LINEAGE_CONTROL.tsv`); across-seed SD is **0.005064** against a primary estimate of **−0.005507**, i.e. seed variation is the size of the effect (`SEED_VARIANCE_SUMMARY.tsv`); and the selected generator's null coverage is 0.900, a 0.100 false-exclusion rate. Outcome stays **BOUND**, and the bound is weaker than this table first implied. |
| review §1.3, permutation control as a bound | **weakened** (E3). |
| review §3 item 11, monotone counterfactual decay | ⚠️ **SUPERSEDED — see `tasks/T-A2-ladder-population/CORRECTION_01`.** "Supported" is **withdrawn**. A stable monotone ladder is **refuted**, not supported: the published ordering holds only under its original weighting and changing populations. On the fixed 423-component set C4 (0.0015582978723404255) **exceeds** C3 (0.0007958345153664303), and both monotonicity flags are `False`. Which design axis causes the shape is **unknown** — three axes vary and only one was fixed. |
