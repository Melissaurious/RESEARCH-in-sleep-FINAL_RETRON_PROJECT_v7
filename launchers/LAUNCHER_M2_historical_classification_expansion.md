...
> approved.**
> - Extraction contract: **MCC-v3.1** (§4a), approved by the operator on 2026-09-19. It
>   replaced MCC-v2 **before any execution**, because newly audited historical source
>   material (Toro 2014 RT0–RT7 extracts) gave a stronger, provenance-backed interval
>   definition.
...
- **K1 — reference not reconstructable.** M2a stops, and there is **no placement at all**, if
  ...
  - fewer than 90 % of clean published-accession proteins yield an extractable MCC-v2 core
    ...
    *Rev-4 note, not a change:* this criterion was written for MCC-v2 and stays as written.
    MCC-v3.1 extracts 1,490 / 1,814 (82.1 %), with clade-dependent loss (clade 10: 72/164).
    No new threshold is set after seeing that number. Instead, every M2a result is reported
    for both extractors, and the extraction difference is itself a result.
...
### 4a. The operational interval — MCC-v3.1 (FROZEN) primary; MCC-v2 required core as sensitivity

**Frozen specification.** It is `analysis/mestre_audit/scripts/m10_mcc_v3_freeze.py`, with
parameters in `m2_design/mcc_v3/MCC_V3_PARAMS.json` (script sha256 recorded inside).
Terminology: the Toro 2014 alignment is historical source material; it is **not** a Mestre MSA.
No Mestre clade label enters template choice, extraction, alignment or placement.

| rule | frozen value | basis |
|---|---|---|
| R1 template eligibility | the 102 Toro 2014 Table S1 "Retrons" extracts. Historical validation only: templates ≥ 0.90 identity to the query are excluded | Toro 2014 predates the Mestre labels |
| R2 similarity metric | MMseqs2 `-s 7.5`, e ≤ 1e-5; `fident`, and template coverage = aligned template span / template length | declared |
| R3 ties | best bitscore; within 0.5 % → higher template coverage → smallest template id; the tie count is recorded | declared |
| R4 minimum similarity | `fident` ≥ **0.3503** and template coverage ≥ 0.80 | 5th percentile of nearest-neighbour identity within the Toro retron set (Toro data only) |
| R5 maximum boundary uncertainty | the start/end spread among templates scoring ≥ 0.90 of the best must be ≤ **16 aa** | 95th percentile (16.5, rounded half-to-even) of that same spread within the Toro retron set |
| R6 no acceptable template / QC failure | `UNABLE_TO_EXTRACT_HISTORICAL_CORE_RELIABLY` + reason (`NO_HIT`, `LOW_IDENTITY`, `LOW_TEMPLATE_COVERAGE`, `DISCORDANT_TEMPLATES`, `CORE_QC_FAILED`) | never forced; never evidence of a non-retron |
| orthogonal QC | the MCC-v2 hmmalign route places ≥ 70 % of required-core states (blocks 4–28) inside the interval | operator |

**Recorded per sequence:** chosen template, template identity, template coverage, projected
start and end, boundary uncertainty (start and end), tie count, number of concordant templates,
discordance flag, MCC-v2 status and core-inside-interval fraction.

**v3.0 → v3.1, the one correction.** It was found after the first application:
- v3.0 calibrated R5 on a different quantity (the best template's deviation, giving 10 aa)
  from the one R5 applies (the concordant-template spread);
- v3.1 calibrates the applied quantity, from Toro data only;
- v3.0 outputs are kept in `mcc_v3/superseded_v3.0/`. There is no further change.

**Historical application (v3.1):**
- clean: **1,490 / 1,814 extracted**. Failures: 296 `DISCORDANT_TEMPLATES`,
  27 `LOW_IDENTITY`, 1 `CORE_QC_FAILED`;
- RNA-polymerase substitutes: 0 / 15;
- other substitutes: 48 / 97.

**Direct validation of the instrument** (the 76 source-stated proteins, leave-near-identical-
template-out; reporting only, no threshold was tuned):
- 62 / 76 extracted;
- start / end within 5 aa: 0.710 / 0.855;
- median error 0 / 0 aa.

**Clade-dependent extraction** (evaluation-only denominator,
`MCC_V3_v2_extraction_by_clade_EVALUATION_ONLY.tsv`): clade 10 retains 72 / 164 under v3.1
(164 / 164 under MCC-v2); clade 8 293 / 406; clade 2 86 / 111. Every M2a result is reported
under both extractors.

---

*Record of the choice as presented to the operator (rev-3):*

**Why this was open.** The Toro 2014 source audit (M1 §14–17) found **source-stated RT0–RT7
