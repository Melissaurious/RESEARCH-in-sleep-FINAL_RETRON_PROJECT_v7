...
good = [b for b, _, _ in blocks if occ[b] >= 0.5]
...
def core_bounds_cols(colres):
    ...
    s = [r for c, r in colres.items() if c >= bcols[N_ANCHOR][0] and c <= COL_HI]
...
for t in sorted(seqs, key=lambda x: int(x.split("_")[1])):
    ...
    b1 = (min(c1), max([st[k] for k in WIN if k in st and state_col[k] <= COL_HI])) if c1 else (None, None)
    ...
    rows.append([t, qset[t], len(seqs[t]), status, start or "", end or "", len(ext), round(fr(N_STATES), 3),
                 round(fr(C_STATES), 3), round(fr(CORE), 3), rt0_res, b1[0] or "", b1[1] or "", b2[0] or "", b2[1] or "", ds, de])
