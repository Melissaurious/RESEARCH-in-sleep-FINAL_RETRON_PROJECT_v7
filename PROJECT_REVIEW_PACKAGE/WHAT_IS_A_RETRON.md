...
**Package edition, 2026-09-19.** Incorporates Stage 3C (`34000ee`) **as corrected by its independent
review and errata** (`eeaf0ae`), and `embed_x2` (results `4f8550b`, closure `fdf0872`).

⚠️ **Two Stage-3C-derived statements below are weaker than they first appeared**, and are written at
their corrected strength: the catalytic-centre landmark rests on **non-retron** chains with a
**7/7-from-two-groups** replicate basis (not 13/14), and `CAT_STATE 262`'s two-residue offset is an
**expected motif-identity check**, since that state was constructed as the modal HMM state of
`[YF].DD` motif starts. Region X's reported motif hits are **withdrawn** (undeclared window).

Annotation lineage is how the *population* is defined; it is not a biological determinant of what a
retron is.
...
| Catalytic RT core (YxDD / palm Asp) | **yes** | yes (all RTs) | no | no | **yes, for RT-ness** | ESTABLISHED, not discriminative. **Stage 3C observed a candidate architectural landmark, at corrected strength:** catalytic aspartates lie in a single structural unit in 19/19 chains (in-sample co-location, detector thresholds calibrated on these same pairs); under the *declared* truth-bearing replicate scope the site-containing unit is stable in **7/7 pairs from only TWO biological groups** (median 0.882) against **0/7** for unit count — the widely quoted 13/14 is an exploratory hybrid. `CAT_STATE 262` sits 2 residues before the catalytic Asp in 17/17 chains, but that state was **constructed** from `[YF].DD` motif starts, so this is a motif-identity check. ⛔ **All 19 truth-bearing chains are non-retron; no retron catalytic-architecture conclusion follows** |
...
| Retron-specific X / Y regions | **no** (Y is not universal) | Y defined in **15/21** retron chains; **X in only 3/21** under the declared rule | partly — but **not exclusive** | **yes** — no source states a general numbered interval for either | not as a motif | **PRELIMINARY (Y) / WITHDRAWN-and-UNDERPOWERED (X)**. Y: rule frozen before scanning; **all 6 Retron-Eco8 chains lack a VTG-like triplet**; VTG appears in **4 non-retron** chains; Y median length 74 aa vs the literature's "~90 aa"; Y coincides with the Ec86 **thumb** (238–320) and its contact-concentration ratio is **1.34×** its chain-length fraction (median over 15 chains from **five** groups, range 0.59–1.86) vs 1.06 over **three** non-retron RNA-contacting chains — **a location ratio, not enrichment evidence for specificity**. X: under the declared two-block rule X is defined in **24/62** chains and **none of those 24 intervals contains NAXXH or AXXH**; all reported motif hits came from an undeclared wide-window fallback and are withdrawn. ⚠️ group II "domain X" is the **thumb**; retron "Region X" is between RT2 and RT3 — different objects |
...
- **Operationally at genome scale:** a retron is best reported as a multi-axis profile whose ncRNA
  ...
  the best candidate architectural landmark is the **catalytic centre inside a single core unit** —
  observed on a selected, **non-retron** truth set with a two-group replicate basis, and therefore a
  hypothesis for retrons rather than an established fact about them.
