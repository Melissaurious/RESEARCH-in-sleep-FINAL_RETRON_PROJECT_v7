...
independent review, four claim rows are withdrawn, and five repairs are unauthorised — so the
architecture paper cannot be assembled from it as it stands.

No option is forced: A–D and C2 are laid out with their evidence and their risks, and the choice is
the operator's.
...
## Option C2 · RT architecture across experimental structures — **BLOCKED pending Stage-3C repairs**

Stage 3C is paper-shaped but **not currently citable**: independent review `FAIL_BLOCK` 4.5/10, five
blockers upheld, rows S05/S11/S12 and the Region-X part of S16 withdrawn, and `STAGE3C_DECISION_REPORT.md`
may not be read without its errata. Writing this paper requires authorising and running **R1–R5**
first. What follows is what the material could support *after* those repairs, at corrected strength.
...
**Evidence, at corrected strength:** catalytic Asps in a single unit 19/19 and detector prediction in
the same unit 19/19 — but **in-sample** (thresholds calibrated on those pairs). Under the declared
truth-bearing scope the replicate stability is **7/7 from TWO biological groups** (median 0.882) vs
**0/7** for unit count; the 13/14 figure is an exploratory hybrid. `CAT_STATE 262`'s 2-residue offset
in 17/17 chains is an **expected motif-identity check** (the state was built from `[YF].DD` motif
starts). State blocks **merge** (SB3+SB56 28/28, SB56+SB7 35/36) — unaffected by the blockers.
Literature fingers coincide **0/8** after the motif row is excluded; two 2026 papers on the same Eco8
protein publish **incompatible** partitions; the inherited boundary product is RED on its own files
(residual construction 25/25; the 5G2X palm excludes that chain's own catalytic Asp). **All 19
truth-bearing chains are non-retron.**
...
**Risks:** the bundle currently carries a FAIL_BLOCK review; 62 chains from a **selected** register
(assembled downstream of Stage-3B catalytic-evidence collection), 31 groups in one structural
cluster, cryo-EM 50/62; Comparison A is **retron-free**; Stage 3B's mislabel must be repaired first,
since this paper would cite it.
