...
**Package edition, 2026-09-19.** Incorporates `embed_x2` (results `4f8550b`, closure `fdf0872`) and
Stage 3C **as corrected by its failed independent review** (`eeaf0ae`).

⚠️ **Stage 3C's own negatives are now partly self-inflicted**: its independent review returned
`FAIL_BLOCK` 4.5/10 with five blockers, all upheld, and two of its headline numbers were computed
under rules the governing launcher does not declare. Those corrections are folded into §2a below.
...
| result | value | kind |
|---|---|---|
| **Literature fingers coincide with a structural unit** | **0 of 8** after excluding a motif row wrongly counted as a boundary (median best Jaccard 0.488); originally reported as 0/11 | REFUTED — the literature does not pin these down either |
...
| Region X coverage (**corrected**) | under the launcher-declared rule X is defined in **24/62** chains and **3/21 retron** chains, and **none of those 24 intervals contains NAXXH or AXXH**. The originally reported 33/62 coverage and the 4 strict / 2 relaxed motif hits came from an **undeclared wide-window fallback** and are **WITHDRAWN** (blocker B4) | UNDERPOWERED + a process failure |
...
**The two findings once offered as positives are weaker than reported, and are recorded here at
corrected strength.** (i) The catalytic site lies in a single unit 19/19 and the detector's
prediction shares that unit 19/19 — but the detector's thresholds were **calibrated on these same
truth pairs**, so this is in-sample co-location, not validation. Under the *declared* truth-bearing
replicate scope the site-containing unit is stable in **7/7 pairs from only TWO biological groups**
(median 0.882) against **0/7** for unit count; the widely quoted **13/14 is an exploratory hybrid**
of seven truth-based and seven detector-based pairs (blocker B1). (ii) `CAT_STATE 262` sits 2
residues before the catalytic aspartate in 17/17 chains — but that state was **constructed** as the
modal HMM state of `[YF].DD` motif starts, making this an **expected motif-identity check, not
independent validation** (erratum E-3C-4). Scope, unchanged and decisive: **all 19 truth-bearing
chains are non-retron, so no retron catalytic-architecture conclusion follows.**

| Stage-3C process negative | value | kind |
|---|---|---|
| **Independent review of the Stage-3C bundle** | Codex, model-disjoint, read-only: **`FAIL_BLOCK` 4.5/10, five blockers, all upheld** after the author re-derived every contested number | REFUTED as promotable |
| Consequence | claim rows **S05, S11, S12 and the Region-X part of S16 WITHDRAWN**; five re-runs (R1–R5) specified but **not authorised**; the report and README **may not be cited without the errata** | — |
...
> near-neighbour discrimination at 52.5 % of pairs. The best candidate architectural landmark is a
> catalytic centre occupying a single structural unit (19/19 chains, in-sample; replicate-stable in
> 7/7 pairs from only two biological groups under the declared scope) — observed on **non-retron**
> truth chains, and therefore a hypothesis for retrons rather than an established fact about them.
