...
from mapper import (BIN, run, hmm_leng, state_to_residue, read_stockholm,
                    domain_scores, classify_sequence)
from loader import load_families, authorised_families
...
def read_fasta(p):
    ...
    for line in open(p, errors="replace"):
        line = line.rstrip()
        if line.startswith(">"):
            n = line[1:].split()[0]; s[n] = []
        elif n is not None:
            s[n].append(line.strip())
    return {k: "".join(v) for k, v in s.items()}

# REPAIR 4: the reason-code vocabulary is CLOSED. A verdict/reason pair outside this set is a
# defect, not a new category. Tests assert exact membership, not mere presence of abstention.
REASON_VOCABULARY = {
    ("MAPPED", "OK"),
...
elig = load_families([f for f in authorised_families()
                      if f in ("Retrons", "UG3", "AbiA")])
probe = {}
...
# REPAIR 4: assert the EXACT reason code, not merely that abstention happened. v1 checked only
# `verdict == ABSTAIN`, so a mutant emitting the SAME reason for both junk and the U10b
# fragment passed U10 and U10b together. These now pin the exact pair on each side.
junk_exact = all(verd[k][0] == "ABSTAIN" and verd[k][1] == "NO_QUALIFYING_DOMAIN"
                 for k in junk)
cons_exact = (vcons[0], vcons[1]) == ("MAPPED", "OK")
in_vocab = all((v[0], v[1]) in REASON_VOCABULARY for v in list(verd.values()) + [vcons])
rec("U10_abstention_exact_reason_codes", "FALSIFIABLE_EMPIRICAL_TEST",
    junk_exact and cons_exact and in_vocab,
    f"junk={ {k: (v[0], v[1]) for k, v in verd.items()} }; consensus={(vcons[0], vcons[1])}; "
    f"all pairs in closed vocabulary={in_vocab}",
    "both junk sequences return exactly (ABSTAIN, NO_QUALIFYING_DOMAIN), the consensus returns "
    "exactly (MAPPED, OK), and every pair is in the closed vocabulary",
    "the right verdict with the WRONG reason code, a reason outside the vocabulary, or a "
    "required abstention silently converted to MAPPED",
    "hmmemit consensus -> (MAPPED, OK)", "random + poly-A -> (ABSTAIN, NO_QUALIFYING_DOMAIN)")

# ---- U10m MUTANT CONTROLS: prove the assertions above can actually catch each failure -----
# Each mutant is a deliberately wrong classifier run against the SAME inputs. If a mutant
# passed the U10/U10b predicates, those predicates would be decorative.
def _pred_u10(fn):
    """The U10 predicate, applied to an arbitrary classifier."""
    v = {k: fn(m10[k], ANCH, leng, S_MIN, K_MIN, dom10.get(k)) for k in junk}
    vc = fn(m10b["cons"], ANCH, leng, S_MIN, K_MIN, dom10b.get("cons"))
    return (all(v[k][0] == "ABSTAIN" and v[k][1] == "NO_QUALIFYING_DOMAIN" for k in junk)
            and (vc[0], vc[1]) == ("MAPPED", "OK")
            and all((x[0], x[1]) in REASON_VOCABULARY for x in list(v.values()) + [vc]))


def mutant_wrong_reason(states, anchors, leng_, s_min, k_min, dom):
    v = classify_sequence(states, anchors, leng_, s_min, k_min, dom)
    # swaps the two abstention reasons - the exact mutant v1 could not detect
    if v[0] == "ABSTAIN":
        swap = ("INSUFFICIENT_SUPPORTED_ANCHORS" if v[1] == "NO_QUALIFYING_DOMAIN"
                else "NO_QUALIFYING_DOMAIN")
        return (v[0], swap) + v[2:]
    return v


def mutant_never_abstains(states, anchors, leng_, s_min, k_min, dom):
    v = classify_sequence(states, anchors, leng_, s_min, k_min, dom)
    return ("MAPPED", "OK") + v[2:]


def mutant_out_of_vocabulary(states, anchors, leng_, s_min, k_min, dom):
    v = classify_sequence(states, anchors, leng_, s_min, k_min, dom)
    return (v[0], "LOW_CONFIDENCE") + v[2:] if v[0] == "ABSTAIN" else v


caught = {"wrong_reason": not _pred_u10(mutant_wrong_reason),
          "never_abstains": not _pred_u10(mutant_never_abstains),
          "out_of_vocabulary": not _pred_u10(mutant_out_of_vocabulary)}
rec("U10m_mutant_classifiers_are_caught", "FALSIFIABLE_EMPIRICAL_TEST",
    all(caught.values()) and _pred_u10(classify_sequence),
    f"mutants caught={caught}; the real classifier still passes={_pred_u10(classify_sequence)}",
    "all three deliberately-wrong classifiers FAIL the U10 predicate while the real one passes",
    "any mutant slips through - which would mean U10 asserts nothing",
    "the real classify_sequence (must pass)",
    "swapped-reason, never-abstains and out-of-vocabulary mutants (all EXECUTED)")
...
rec("U10b_second_abstention_reason_exact", "FALSIFIABLE_EMPIRICAL_TEST",
    (v_frag[0], v_frag[1]) == ("ABSTAIN", "INSUFFICIENT_SUPPORTED_ANCHORS")
    and (v_frag[0], v_frag[1]) in REASON_VOCABULARY
