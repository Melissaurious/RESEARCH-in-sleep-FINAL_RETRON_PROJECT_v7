#!/usr/bin/env python3
"""Hash every EXTERNAL input this bundle depends on.

The manifest covers files inside the bundle. These live outside it, and a silent change to
any one would change every result while the bundle manifest still verified clean.
"""
...
for b in BINS:
    rows.append([f"binary_{b}", BIN + b, sha256(BIN + b), str(os.path.getsize(BIN + b))])
