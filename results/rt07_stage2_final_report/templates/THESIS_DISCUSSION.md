...
*Thesis chapter section. Source of record `{{pinned_short}}`.*
...
operational coordinate system of {{n_anchors}} conserved profile-HMM states that is validated,
versioned (`{{mapper_version}}`) and applied to {{g5a_eligible}} catalogue RTs. The historical
...
{{g7_anchor_span}}, which starts C-terminal of both labels' reference intervals. Each reason names the evidence that could change the status: the primary
...
- **N-terminal blind spot** of the instrument (anchors cover LtrA {{g7_anchor_span}}).
- **Validation scope.** Transfer is shown to one fresh lineage under `{{profile_m}}`. Not shown:
...
- **g6 statistics.** Ordinal tie-breaking; nulls too small for 1% tails; no multiplicity control;
  {{g6_strata_excluded}} of {{g6_strata_total}} subtype strata excluded; post-hoc procedural
  additions disclosed.
- **Human input audit** is {{hia_pending}} for the landed bundles.
