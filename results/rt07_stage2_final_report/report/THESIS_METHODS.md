...
*Thesis chapter section. Numbers are resolved from the landed Stage-2 record at `94a1a78`;
...
6 held sources. Each of 32 region names was scored against each source
...
quotation (36) was checked verbatim against the source PDF. Citation genealogy was
traced to separate original statements from restatements. The Zimmerly group-II-intron RT
alignment (ALIGN_000044; 66 sequences, 1,441 columns) was acquired and
verified as the historical reference substrate.

## Independent reconstruction of the historical landmarks

Conserved blocks were reconstructed on the 66 reference proteins in two alignment
...
13 RT0–RT7 claims from earlier project work were regenerated from their inputs and
...
471) with `hhmake -M 50` as the match-state convention. The instrument uses
150 frozen conserved states: the states aligned in every construction family ("all
...
residue: MAPPED if the posterior was ≥ PP_HI (0.75), AMBIGUOUS if it lay between PP_LO
(0.5) and PP_HI, UNSUPPORTED if it was lower, and DELETED_STATE if the alignment path
skipped the state. A sequence was committed only if its domain bitscore exceeded S_MIN
(10) and at least K_MIN (30) anchors were MAPPED. Otherwise it abstained with a
...
bitscore. K_MIN: strictly above the highest negative-control MAPPED-anchor count. T1 (0.32), the fifth
percentile of per-sequence MAPPED fraction over construction, was the transfer floor. D_MAX
(0.48) bounded the difference between component medians, with a same-population random-split
companion D_RANDOM (0.067) reported alongside. Calibration used six construction families.
...
Controls were 252 shuffles of real UG25 RTs in three classes: MONO (composition
...
and the domain-scoring protocol. It is `rtmap-1.0.0/53a1e738a19b3896`. Eligibility (≥ 250 residues,
20 standard amino acids) was applied once, in a separate census of the 501,561 exact RT
sequences of the Stage-1 catalogue. That census fixed the denominator at 369,381. The
