#!/usr/bin/env python3
"""rt07_g1 step 7 - assemble REPORT.md and a self-contained REPORT.html.
...
resolution trace is landed as g1_resolved_values.tsv so any number in the report can be
...
Writes: REPORT.md, REPORT.html, tables/g1_resolved_values.tsv
"""
...
def resolve(tables: Path, trace: list) -> dict[str, str]:
    ...
    for key, (table, sel, col, kind) in F.VALUES.items():
        ...
        trace.append({"key": key, "bundle": "rt07_g1_history_and_definition",
                      "table": table,
...
def main() -> int:
    ...
    write_tsv(tables / "g1_resolved_values.tsv",
              ["key", "bundle", "table", "selector", "column", "raw_value", "rendered",
               "unit", "denominator"], trace)

    md = [f"# {F.TITLE}", "", f"_{F.SUBTITLE}_", "",
          "Every number below resolves to a landed table via `tables/g1_resolved_values.tsv`. "
    ...
    (args.out / "REPORT.html").write_text(
        ...
        "<code>tables/g1_resolved_values.tsv</code>. This page computes nothing.</p>"
