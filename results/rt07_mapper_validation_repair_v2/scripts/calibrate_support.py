...
# REPAIR 5: family-scoped loader. The old eligible_by_family() materialised all 38 families,
# UG25 included, which falsified the "UG25 was never read" claim.
...
def pctl(vals, q):
    """Percentile by the same index convention used in the PP_HI sweep."""
    v = sorted(vals)
...
def main():
    ...
    with open(f"{TABLES}/negative_control_summary.tsv", "w") as f:
        ...
        f.write("# OPTIONAL-IMPLEMENTED-NOW: per-control-class p95 is reported alongside the\n"
                "# pooled figure. The pooled p95 of 0 HID the reverse class, whose own p95 is 10.\n")
        f.write("family\tcontrol_kind\tn\tmax_mapped_anchors\tmean_mapped_anchors\t"
                "p95_mapped_anchors\tn_with_any_mapped\tmax_domain_bitscore\n")
