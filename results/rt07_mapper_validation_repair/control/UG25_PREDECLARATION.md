...
**No UG25 mapping result has been inspected.** No UG25 sequence has been aligned, scored or
read by any script in this bundle. The only UG25 facts used are its eligible count and component
sizes, taken from `fresh_lineage_candidate_audit.tsv`, which was produced before this repair and
contains **no mapping performance** — they are used for **sampling feasibility only**.
...
| C3 | catalytic | among sequences whose `CAT_STATE` is `MAPPED`, the fraction beginning a `[YF].DD` is **≥ 0.80** (the construction agreement bound) | < 0.80 |
...
| C6 | decoy separation | max negative `MAPPED` **<** min real `MAPPED` among non-abstaining sequences, **and** negative 95th percentile **= 0** | any overlap, or p95 > 0 |
